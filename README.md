...
5. **移動時間不足チェック**
   - 同一職員の連続サービスで住所が異なる場合、30分以上の間隔が必要
   - 不足している場合はエラーフラグを付与
...
- アプリ画面右上に「最終更新日時」を表示しています。
- `streamlit_app.py` の `APP_LAST_UPDATED` を **git push 前に必ず更新** してください。
...
- **補足**：`サービス提供責任者` 列は住所が入っている前提で参照（移動時間チェック用）
